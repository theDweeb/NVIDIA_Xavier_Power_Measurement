...
from .activations import sigmoid, hard_sigmoid, HardSigmoid
...
class SqueezeExcite(nn.Module):
    def __init__(self, in_chs, se_ratio=0.25, reduced_base_chs=None,
                 act_layer=nn.ReLU6, gate_fn=sigmoid, divisor=1, **_):
        super(SqueezeExcite, self).__init__()

        self.gate_fn = HardSigmoid(inplace=True) #gate_fn
        reduced_chs = make_divisible((reduced_base_chs or in_chs) * se_ratio, divisor)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.conv_reduce = nn.Conv2d(in_chs, reduced_chs, 1, bias=True)
        self.act1 = act_layer(inplace=True)
        self.conv_expand = nn.Conv2d(reduced_chs, in_chs, 1, bias=True)
        #self.mult = distiller.modules.EltwiseMult(inplace=False)
...
class ConvBnAct(nn.Module):
    def __init__(self, in_chs, out_chs, kernel_size,
                 stride=1, dilation=1, pad_type='', act_layer=nn.ReLU6,
...
class DepthwiseSeparableConv(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, dw_kernel_size=3,
                 stride=1, dilation=1, pad_type='', act_layer=nn.ReLU6, noskip=False,
        ...
        self.conv_dw = nn.Sequential(
            select_conv2d(in_chs, in_chs, dw_kernel_size, stride=stride, dilation=dilation, padding=pad_type, depthwise=True),
            norm_layer(in_chs, **norm_kwargs),
        ...
        self.conv_pw = nn.Sequential(
            select_conv2d(in_chs, out_chs, pw_kernel_size, padding=pad_type),
            norm_layer(out_chs, **norm_kwargs),
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, dw_kernel_size=3,
        ...
        # New BN fuse
        self.conv_pw = nn.Sequential(
            select_conv2d(in_chs, mid_chs, exp_kernel_size, padding=pad_type, **conv_kwargs),
            norm_layer(mid_chs, **norm_kwargs),
            act_layer(inplace=True)
        )
        ...
        # New BN fuse
        self.conv_dw = nn.Sequential(
            select_conv2d(mid_chs, mid_chs, dw_kernel_size, stride=stride, dilation=dilation, padding=pad_type, depthwise=True, **conv_kwargs),
            norm_layer(mid_chs, **norm_kwargs),
            act_layer(inplace=True)
        )
        ...
        self.conv_pwl = nn.Sequential(
            select_conv2d(mid_chs, out_chs, pw_kernel_size, padding=pad_type, **conv_kwargs),
            norm_layer(out_chs, **norm_kwargs)
        )
...
class EdgeResidual(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, exp_kernel_size=3, exp_ratio=1.0, fake_in_chs=0,
                 stride=1, dilation=1, pad_type='', act_layer=nn.ReLU6, noskip=False, pw_kernel_size=1,
                 se_ratio=0., se_kwargs=None, norm_layer=nn.BatchNorm2d, norm_kwargs=None,
                 drop_connect_rate=0.):
